...
<div><font size="7">项目名称:${projectDetail.title }</font></div>
	<table style="border: 1px black solid;">
		...
		<c:forEach items="${projectLog }" var="log" varStatus="index">
			<tr <c:if test="${index.index%2==0 }">style="color:gray"</c:if>>
				...
				<td>
					<c:choose>
						...
						<c:when test="${log.preStatus==2 }">(开发中)</c:when>
				...
				<td>
					<c:choose>
						...
						<c:when test="${log.afterStatus==2 }">(开发中)</c:when>
